...
\chapter{Analysis}
	...
	\par Not all modeling languages are executable, and for those that are, the use of them does not necessarily mean that programmers are no longer required. On the contrary, executable modeling languages are intended to amplify the productivity of skilled programmers, so that they can address more challenging problems, such as parallel computing and distributed systems\cite{wikipedia}.
		...
		\par It is necessary to follow a work flow when using this DSL to create the necessary artifacts. Next the correct work flow is explained.
			...
			\begin{figure} [!h]
				\centering
				\includegraphics[width=0.5\linewidth]{Images/use-cases}
				\caption{Example diagram.}
				\label{fig:use-cases}
			\end{figure}			
...
\chapter{Design}
	...
	\par The first step is to analyze the \texttt{main.cpp} file. It is created an object of the \texttt{DBTEngine} type and the code size and location are passed as arguments to the initialization (\texttt{initTranslator}) method. Hereupon is only necessary to run the translator (\texttt{runDBT}).
	\par This takes to the first identified component: the \texttt{DBTEngine}. The functions \texttt{runDBT} and \texttt{initTranslator} form an interface and can be identified as a service that the top level reference. For this, there must be code into translation cache. If this cache is empty, a basic block stored into code cache must be translated. Here are identified one reference, one service and another component: \texttt{DBTEngine} makes a reference to a \texttt{TranslationCache} service. 
	...
	\section{Reference Architecture}
		...
		\subsection{Source Cluster}
		
			\subsubsection{Source Architecture}
		
			\subsubsection{Source Environment}
		
				\subsubsubsection{Data Memory}
		
			\subsubsection{Decoder}
		
		
		\subsection{Target Cluster}
