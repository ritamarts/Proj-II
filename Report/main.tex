...
\lstset{emph={interface, compile, language, component, properties, services, references, subcomponents, bind, to, annotation, int, string, float, promote, service, reference, is}, emphstyle=\color{purple}\textbf,
	tabsize=1,
	frame=single,
	breaklines=true,
...
\chapter{Design}
	...
	\begin{figure} [H]
		\centering
		\includegraphics[width=0.9\linewidth]{Images/refactoringuml}
	...
	\begin{figure} [H]
		\centering
		\includegraphics[width=1\linewidth]{Images/codegeneration}
...
\chapter{Implementation Phase}
	...
	The Scripts of the DBT were made following the Reference Architecture. They were organize in the files with the extension el with the following conventions, made by the groups involve in the modeling of the DBT: 
	\begin{itemize}
		\item The \texttt{components} are placed in script with the same name and following the rule one component by script. 
		\item The \texttt{interfaces} are placed in a single script, with the name "Interfaces", and import in each component script.
		\item The \texttt{language} is defined in a single script, with the name "Languages" and also is imported in each component script.
		\item The subcomponents should be imported as well.
		\item The scripts name are written with Upper Camel Cases.
	\end{itemize}
	
	The result is shown in the figure \ref{fig:scripts}.
	
	\begin{figure} [H]
		\centering
		\includegraphics[width=0.2\linewidth]{Images/scripts}
		\caption{List of EL Scripts for the DBT}
		\label{fig:scripts}
	\end{figure}
	
	Starting from the language script that is imported in the other files and defines the annotation that is used in the source files.
	\begin{lstlisting}
	language C
	{
		annotation: "@@"
	...
	\end{lstlisting}
	The Component generator were made following the topics that were showed before. From the reference architecture we can take the properties, services and references that should be defined in each component.
	...
	The binds of the generator to other components that are in the same level of him are made in the component that has the generator has subcomponent, which means the script \texttt{TargetCluster.el}.
	
	As we can observe in the line 12 and 13 are made the wires between component generator and the target architecture.
	
	In the lines 15, 16, 17 and 18 are promoted the references to be bind to other components above the level of the generator.
	...
	component TargetCluster (C)
	{
		...
		promote reference gen.r_EngineState  as pr_EngineState
	...
	\end{lstlisting}
	Finally, in the top level are the binds for the service and references promoted from the component Generator and connected to different components through out each interface. This is showed in the next code in the lines from 49 to 54. 
	\begin{lstlisting}
	import "Languages.el"
	import "Interfaces.el"
	...
	component DBT (C)
	{
		...
		bind trgCluster.pr_EngineState to dbtEngine.s_EngineState 
